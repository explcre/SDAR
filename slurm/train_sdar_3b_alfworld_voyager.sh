#!/bin/bash
# SDAR 3B ALFWorld from-scratch TRAINING on voyager (4x H100) - retry of the galaxy attempt that
# OOM'd/crashed on tight 3090s. H100 80GB + eager mode = ample headroom. Reuses parameterized trainer.
# Submit: sbatch --job-name=sdar3b_train_voy --nodelist=voyager slurm/train_sdar_3b_alfworld_voyager.sh
#SBATCH --nodes=1
...
export NGPUS=2 TP=2 GMU=${GMU:-0.5} \
  ...
  EXPNAME=sdar_qwen2.5_3b_repro_voy \
  OUTDIR=$SSD/sdar_train/sdar_qwen2.5_3b_repro_voy
