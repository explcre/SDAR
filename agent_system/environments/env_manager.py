...
from agent_system.memory import SimpleMemory, SearchMemory, MathToolMemory
...
class MathToolEnvironmentManager(EnvironmentManagerBase):
    """EnvironmentManager for the math_tool (AIME tool-integrated-reasoning) env.

    Mirrors ``SearchEnvironmentManager``: keeps a ``MathToolMemory`` so
    ``build_text_obs`` shows the running multi-turn transcript (original problem
    + each (model action, tool response) pair), letting the model see prior
    turns. The model emits a hermes ``code_interpreter`` tool call or a final
    ``\\boxed{}`` answer; the env executes code via SandboxFusion or scores the
    answer with the SOD strict-box scorer.
    """
    ...
    def build_text_obs(
        ...
        """Build the per-env prompt: SOD problem (+ running transcript when enabled)."""
        postprocess_text_obs: List[str] = []

        if not init and self.config.env.history_length > 0:
            memory_ctx, _ = self.memory.fetch(
                self.config.env.history_length,
                obs_key="tool_response",
                action_key="action",
            )

        for i in range(len(text_obs)):
            if init or self.config.env.history_length <= 0:
                obs_i = MATH_TOOL_TEMPLATE_NO_HIS.format(
                    task_description=self.tasks[i]
                )
            else:
                obs_i = MATH_TOOL_TEMPLATE.format(
                    task_description=self.tasks[i],
                    memory_context=memory_ctx[i],
                    step_count=len(self.memory[i]),
                )
    ...
    def _process_batch(self, batch_idx, total_batch_list, total_infos, success):
        # Find the last entry with active masks (mirrors SearchEnvironmentManager).
        for i in reversed(range(len(total_batch_list[batch_idx]))):
            batch_item = total_batch_list[batch_idx][i]
            if batch_item['active_masks']:
                info = total_infos[batch_idx][i]
...
def make_envs(config):
    ...
    elif "math_tool" in config.env.env_name.lower():
        from agent_system.environments.env_package.math_tool import build_math_tool_envs, math_tool_projection
        _envs = build_math_tool_envs(seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, is_train=True, env_config=config.env)
        _val_envs = build_math_tool_envs(seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, is_train=False, env_config=config.env)

        projection_f = partial(math_tool_projection)
        envs = MathToolEnvironmentManager(_envs, projection_f, config)
        val_envs = MathToolEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
